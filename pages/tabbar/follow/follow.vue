	...
	<view class="follow">
		...
		<view class="follow-list">
			<swiper class="follow-list__swiper" :current="activeIndex" @change="change">
				...
				<swiper-item>
					<list-scroll>
						<list-author v-for="(item,index) in authorLists" :key="item._id" :item="item"></list-author>
						<view class="no-data" v-if="authorShow">没有关注</view>
					</list-scroll>
				</swiper-item>
			</swiper>
		</view>
		...
		methods: {
			...
			getAuthor(){
				this.$api.get_author().then(res=>{
					...
					this.authorShow = this.authorLists.length === 0 ? true : false
				})
			}
		}
	}
	...
	.follow-tab{
		...
		.follow-tab__box{
			...
			.follow-tab__item{
				...
				font-size: 14px;
				width: 100%;
				&:first-child{border-right:1px $mk-base-color solid ;}
